<template>
  <div id="app">
    <h4 class="bg-primary text-white text-center p-2">{{name}}'s To do App</h4>
    <div class="container-fluid p-4">
      <div class="row" v-if="filteredTasks.length == 0">
        <div class="col text-center">
          <b>Nada pra fazer! aeee</b>
        </div>
      </div>
      <template v-else>
        <div class="row">
      ...
      <div class="row py-2">
        <div class="col">
          <input v-model="newItemText" class="form-control" :placeholder="placeholderItemText" />
        </div>
        <div class="col-2">
      ...
      <div class="row bg-secondary py-2 mt-2 text-white">
        ...
        <div class="col text-center">
          <button class="btn btn-sm btn-warning" v-on:click="deleteCompleted">Delete completados</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      hideCompleted: false,
      newItemText: "",
      placeholderItemText: "Escreva seu Todo aqui"
    };
  },
  computed: {
    filteredTasks() {
